...
(defmulti resolve-nested-tag-values
  ...
  {:added "0.39.0" :arglists '([tag tag-value])}
  (fn [{tag-type :type} _]
    (keyword tag-type)))
...
(defn- get-snippet-name [param-key ex-attrs]
...
(defn- native-snippet-to-ref-map [native-snippet]
  {(str "snippet:" (:name native-snippet))
   (i/map->ReferencedQuerySnippet
     {:snippet-id (:id native-snippet)
      :content    (:content native-snippet)})})

(defn- find-all-nested-snippets [parent-tag level snippet-nm {:keys [snippet-id content], :as snippet} name-to-val]
  (if
