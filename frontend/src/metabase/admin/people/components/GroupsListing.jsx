...
import { TeamApi } from "metabase/services";
import CheckBox from "metabase/components/CheckBox";

// ------------------------------------------------------------ Add Group ------------------------------------------------------------

function AddGroupRow({ team, text, onCancelClicked, onCreateClicked, onToggleTeam, onTextChange }) {
  ...
  return (
    <tr>
      ...
      <td>
        ...
        <button
          className={cx("Button ml2", { "Button--primary": !!textIsValid })}
...
function GroupRow({
  ...
  return editing ? (
    ...
    <tr>
      ...
      <td>{group.collection_id ? <Link className="link no-decoration" to={`/collection/${group.collection_id}`}>Collection</Link> : ""}</td>
      <td className="text-right">
        {showActionsButton ? (
          <ActionsPopover
            group={group}
...
function GroupsTable({
  ...
  return (
    <AdminContentTable columnTitles={[t`Group name`, t`Members`, t`Team?`]}>
      {showAddGroupRow ? (
        <AddGroupRow
          team={team}
          text={text}
          ...
          onToggleTeam={onAddGroupToggleTeam}
        />
      ) : null}
      {groups &&
...
export default class GroupsListing extends Component {
  ...
  async onAddGroupCreateButtonClicked() {
    ...
    try {
      if(this.state.team){
        await TeamApi.create({ name: this.state.text });
    ...
    return (
      ...
      >
        <GroupsTable
          ...
          onAddGroupToggleTeam={this.onAddGroupToggleTeam.bind(this)}
